...
public class GrassPainterWindow : EditorWindow
{
    ...
    public void GeneratePositions(GameObject selection)
    {
        ...
        if (selection.TryGetComponent(out MeshFilter sourceMesh))
        {
            ...
            var meshTriangles = new NativeArray<int>(oTriangles.Length, Allocator.TempJob);
            var meshVertices = new NativeArray<Vector4>(oVertices.Length, Allocator.TempJob);
            var meshColors = new NativeArray<Color>(oVertices.Length, Allocator.TempJob);
            var meshNormals = new NativeArray<Vector3>(oNormals.Length, Allocator.TempJob);
            ...
            var points = new NativeArray<Vector3>(numPoints, Allocator.TempJob);
            var normals = new NativeArray<Vector3>(numPoints, Allocator.TempJob);
            var lengthWidths = new NativeArray<float>(numPoints, Allocator.TempJob);

            // job safe random seeds
            var seeds = new NativeArray<uint>(numPoints, Allocator.TempJob);
            for (int i = 0; i < numPoints; i++)
            {
                seeds[i] = (uint)Random.Range(1, int.MaxValue);
            }
            ...
            JobHandle handle = job.Schedule(numPoints, 64);
            ...
            for (int j = 0; j < numPoints; j++)
            {
                ...
                Vector3 worldNormal = selection.transform.TransformDirection(normals[j]);
    ...
    public void CalcAreas(Mesh mesh)
    {
        ...
        cumulativeSizes = new NativeArray<float>(sizes.Length, Allocator.TempJob);
        total = new NativeArray<float>(1, Allocator.TempJob);
    ...
    private struct MyJob : IJobParallelFor
    {
        ...
        [WriteOnly]
        public NativeArray<float> LengthWidths;
        [WriteOnly]
        ...
        {
            var rng = new Unity.Mathematics.Random(RandomSeeds[index]);

            float randomSample = rng.NextFloat() * Total[0];
            int triIndex = -1;
            for (int i = 0; i < Sizes.Length; i++)
            {
            ...
            switch (VertexFade)
            {
                case SO_GrassToolSettings.VertexColorSetting.Red:
                    ...
                    LengthWidths[index] = 1.0f - ((red + red2 + red3) * 0.3f);
                    break;
                case SO_GrassToolSettings.VertexColorSetting.Green:
                    ...
                    LengthWidths[index] = 1.0f - ((green + green2 + green3) * 0.3f);
                    break;
                case SO_GrassToolSettings.VertexColorSetting.Blue:
                    ...
                    LengthWidths[index] = 1.0f - ((blue + blue2 + blue3) * 0.3f);
                    break;
                case SO_GrassToolSettings.VertexColorSetting.None:
                    LengthWidths[index] = 1.0f;
                    break;
            }
            ...
            Normals[index] = MeshNormals[MeshTriangles[triIndex * 3 + 1]];

            Vector3 RandomPointOnMesh = a + r * (b - a) + s * (c - a);
            Points[index] = RandomPointOnMesh;
        }
    }
